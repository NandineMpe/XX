...
export default function AppSettings({ className, asPage, sidebarClassName }: AppSettingsProps) {
  ...
  if (asPage) {
    return (
      <div className="flex w-full min-h-screen bg-card rounded-xl shadow-lg">
        {/* Sidebar */}
        <nav className={cn("w-64 border-r bg-background rounded-l-xl flex flex-col py-8 px-4", sidebarClassName)}>
              ...
              <li key={tab.key}>
                <button
                  className={cn(
                    ...
                    activeTab === tab.key ? 'bg-primary text-primary-foreground' : 'hover:bg-muted text-foreground'
                  )}
        ...
        {/* Main Content */}
        <div className="flex-1 p-8 overflow-y-auto min-w-0 w-full max-w-none">
          {activeTab === 'general' && (
