...
- Receive status change callbacks from n8n and upsert records
- Allow the UI to poll current request states and download links

Data is persisted to a JSON file under the configured working directory to
survive process restarts in simple deployments. For production persistence,
consider migrating to a database-backed storage.
"""
...
def create_sdr_routes(api_key: Optional[str] = None) -> APIRouter:
    ...
    async def sdr_callback(event: SDRCallbackPayload):
        ...
        raise an error if no usable key can be determined.
        """
