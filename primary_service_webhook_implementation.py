...
def handle_binary_file_upload(
    ...
    try:
        ...
        print(
            f"Binary file uploaded: {request_id}, filename: {filename}, size: {len(file_data)}"
        )
...
def handle_webhook_request(request_data=None, file_data=None, form_data=None):
    ...
    try:
        ...
        if file_data is not None:
            filename = (
                form_data.get("fileName", "uploaded_file")
                if form_data
                else "uploaded_file"
            )
            ...
            return handle_binary_file_upload(
                file_data, filename, request_id, document_type, parameters
            )
