            ...
            try:
                ...
                if health_result["status"] == "success":
                    ...
                    if readiness_result["status"] == "success":
                        ...
                        print(f"[{readiness_result['timestamp']}] ⚠️  Readiness: {readiness_result['error']}")
                    ...
                    if self.api_key:
                        ...
                        if detailed_result["status"] == "success":
                            ...
                            print(f"[{detailed_result['timestamp']}] ✅ Detailed: {data['status']} | Auth: {data['auth_mode']} | Pipeline: {'busy' if data['pipeline_busy'] else 'idle'}")
                        else:
                            print(f"[{detailed_result['timestamp']}] ⚠️  Detailed: {detailed_result['error']}")
                else:
                    consecutive_failures += 1
                    print(f"[{health_result['timestamp']}] ❌ Health: {health_result['error']}")
                    
                    if consecutive_failures >= max_failures:
                        print(f"❌ Service has failed {consecutive_failures} times consecutively. Stopping monitoring.")
                        break
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Monitor LightRAG healthcheck endpoints")
    parser.add_argument("--url", default="http://localhost:9621", help="Base URL of LightRAG API")
    parser.add_argument("--api-key", help="API key for detailed health checks")
    parser.add_argument("--interval", type=int, default=30, help="Check interval in seconds")
    parser.add_argument("--max-failures", type=int, default=3, help="Max consecutive failures before stopping")
    parser.add_argument("--single-check", action="store_true", help="Perform single check instead of continuous monitoring")
